...
export default function AdmissionsPage() {
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[var(--color-bg-light)]">
            ...
            <section className="relative pt-40 pb-20 overflow-hidden bg-[var(--color-brand-red)]">
                {/* Background Details */}
                <div className="absolute inset-0 bg-[url('/assets/hero.jpg')] bg-cover bg-center mix-blend-overlay opacity-20"></div>
                <div className="absolute inset-0 bg-gradient-to-r from-[#E60000] to-transparent"></div>
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                        ...
                        {/* Hero Text */}
                        <FadeIn direction="up">
                            <div className="inline-block bg-white text-[var(--color-brand-red)] font-bold text-xs uppercase tracking-wider px-4 py-1.5 rounded-full mb-6">
                        ...
                        <FadeIn direction="left" delay={0.2}>
                            <div className="bg-white rounded-3xl p-8 md:p-10 shadow-2xl">
                                ...
                                {status === 'success' ? (
                                    <div className="bg-[#FDF2EF] text-[var(--color-brand-red)] p-8 rounded-2xl flex flex-col items-center justify-center text-center">
                                    ...
                                    <form onSubmit={handleSubmit} className="space-y-4">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div className="space-y-1">
                                                ...
                                                <input required type="text" name="name" value={formData.name} onChange={handleChange} className="w-full px-5 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:outline-none focus:border-[var(--color-brand-red)] transition-colors" placeholder="John Doe" />
                                            </div>
                                            <div className="space-y-1">
                                                <label className="text-xs font-bold text-[var(--color-heading)] uppercase tracking-wider pl-1">Phone</label>
                                                <input required type="tel" name="phone" value={formData.phone} onChange={handleChange} className="w-full px-5 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:outline-none focus:border-[var(--color-brand-red)] transition-colors" placeholder="+91 90000 00000" />
                                            </div>
                                        </div>

                                        <div className="space-y-1">
                                            <label className="text-xs font-bold text-[var(--color-heading)] uppercase tracking-wider pl-1">Email</label>
                                            <input required type="email" name="email" value={formData.email} onChange={handleChange} className="w-full px-5 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:outline-none focus:border-[var(--color-brand-red)] transition-colors" placeholder="johndoe@example.com" />
                                        </div>

                                        <div className="space-y-1">
                                            <label className="text-xs font-bold text-[var(--color-heading)] uppercase tracking-wider pl-1">Stream of Interest</label>
                                            <select required name="stream" onChange={handleChange} className="w-full px-5 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:outline-none focus:border-[var(--color-brand-red)] transition-colors text-[var(--color-text)]">
                            ...
                            return (
                                <FadeIn key={idx} direction="up" delay={0.1 * idx}>
                                    <div className="bg-white p-8 rounded-3xl box-shadow-soft border border-gray-100 flex flex-col items-center text-center h-full hover:-translate-y-2 transition-transform">
                                        <div className="w-16 h-16 rounded-full bg-[var(--color-brand-red)]/10 text-[var(--color-brand-red)] flex items-center justify-center mb-6">
                                            <Icon size={28} />
                                        </div>
